    ...
    <button
      ...
      className={`inline-flex items-center gap-1.5 px-3 h-7 rounded-full text-xs font-medium border transition-all ${
        active
          ...
          : 'bg-white border-[#ebebeb] text-[#4d4d4d] hover:text-[#171717] hover:border-[#a1a1a1]'
      }`}
    >
      {label}
      {count !== undefined && (
        <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-mono ${
          active ? 'bg-white/20 text-white' : 'bg-[#f5f5f5] text-[#888888]'
        }`}>
          {count}
        </span>
      )}
...
function SocialIcon({ href, label }: { href: string | undefined; label: string }) {
  ...
  return (
    <a
      ...
      title={label}
      className="inline-flex items-center justify-center w-5 h-5 rounded text-[9px] font-bold bg-[#f5f5f5] text-[#888888] hover:bg-[#ebebeb] hover:text-[#171717] transition-all no-underline border border-[#ebebeb]"
    >
      {abbr[label] ?? label[0]}
    </a>
  );
...
export default function ResultsTable() {
  ...
  function toggleFilter(setter: (v: boolean) => void, val: boolean) { setter(!val); setPage(1); }

  if (loading) {
    return (
      <div className="flex items-center justify-center py-32 text-[#888888] text-sm gap-2">
  ...
  if (leads.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-32 text-center">
        <div
          className="w-16 h-16 rounded-2xl border border-[#ebebeb] bg-[#fafafa] flex items-center justify-center mb-5"
          style={{ boxShadow: '0px 1px 1px #00000005, 0px 2px 2px #0000000a' }}
        >
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="text-[#a1a1a1]">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <StatCard label="With email" value={withEmail.toLocaleString()} sub={`${Math.round((withEmail / leads.length) * 100)}% of total`} accent="text-emerald-600" />
        <StatCard label="With phone" value={withPhone.toLocaleString()} sub={`${Math.round((withPhone / leads.length) * 100)}% of total`} accent="text-blue-600" />
        <StatCard label="With website" value={withWebsite.toLocaleString()} sub={`${Math.round((withWebsite / leads.length) * 100)}% of total`} accent="text-violet-600" />
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 h-8 bg-white border border-[#ebebeb] text-[#171717] placeholder-[#888888] rounded-md text-xs focus:outline-none focus:border-[#171717] focus:ring-1 focus:ring-[#171717]/10 transition-all"
          />
        </div>
      ...
      {/* Table */}
      <div className="rounded-xl border border-[#ebebeb] bg-white overflow-hidden" style={{ boxShadow: '0px 1px 1px #00000005, 0px 2px 2px #0000000a' }}>
                ...
                paginated.map((lead, i) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 whitespace-nowrap text-xs">
                      {lead.phone ? (
                        <a href={`tel:${lead.phone}`} className="text-[#0070f3] hover:text-[#0761d1] transition-colors no-underline">
                          {lead.phone}
                        </a>
                    ...
                    <td className="py-3 px-4 max-w-[160px] truncate text-xs">
                      {lead.email ? (
                        <a href={`mailto:${lead.email}`} className="text-[#0070f3] hover:text-[#0761d1] transition-colors no-underline" title={lead.email}>
                          {lead.email}
                        </a>
                      ) : <span className="text-[#d4d4d4]">—</span>}
                    </td>
                    <td className="py-3 px-4 max-w-[160px] truncate text-xs">
                      {lead.website ? (
                        <a href={lead.website} target="_blank" rel="noopener noreferrer" className="text-[#0070f3] hover:text-[#0761d1] transition-colors no-underline" title={lead.website}>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={safePage === 1}
              className="flex items-center gap-1.5 px-3 h-8 rounded-md border border-[#ebebeb] bg-white text-xs text-[#4d4d4d] hover:text-[#171717] hover:border-[#a1a1a1] disabled:opacity-30 disabled:cursor-not-allowed transition-all"
            >

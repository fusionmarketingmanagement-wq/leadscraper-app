...
export default function ConversationsPage() {
  ...
  function selectConversation(c: (typeof CONVERSATIONS)[0]) {
    setSelected(c);
  ...
  return (
    <div className="mx-auto max-w-6xl min-w-0">
      <div
        className="flex flex-col md:flex-row rounded-xl border border-gray-200 bg-white overflow-hidden min-h-[420px] md:h-[calc(100vh-10rem)] md:min-h-[520px]"
        style={{ boxShadow: '0px 1px 1px #00000005' }}
      >
        <div
          className={`w-full md:w-80 md:shrink-0 border-b md:border-b-0 md:border-r border-gray-200 flex flex-col ${
        ...
        >
          ...
          <ul className="flex-1 overflow-y-auto max-h-[50vh] md:max-h-none">
            {CONVERSATIONS.map((c) => (
              <li key={c.id}>
                <button
                  type="button"
        ...
        <div
          className={`flex-1 flex flex-col min-w-0 min-h-[360px] ${
            mobileChatOpen ? 'flex' : 'hidden md:flex'
          }`}
        >
          <div className="flex items-center justify-between gap-2 p-3 sm:p-4 border-b border-gray-100 flex-wrap">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              <button
                type="button"
                onClick={() => setMobileChatOpen(false)}
                className="md:hidden p-1.5 -ml-1 rounded-lg text-gray-500 hover:bg-gray-100 cursor-pointer shrink-0"
              ...
              <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gray-200 text-gray-600 flex items-center justify-center text-sm font-semibold shrink-0">
                {selected.initials}
              </div>
              <div className="min-w-0">
                <p className="text-sm font-semibold text-[#171717] truncate">{selected.name}</p>
                <StatusBadge label={selected.source} variant="cyan" />
              </div>
            </div>
            <div className="flex gap-2 w-full sm:w-auto">
              <button type="button" className="flex-1 sm:flex-none px-3 h-8 rounded-lg bg-yellow-400 text-yellow-900 text-xs font-medium hover:bg-yellow-500 cursor-pointer">
                Take Over
              </button>
              <button type="button" className="flex-1 sm:flex-none px-3 h-8 rounded-lg border border-gray-200 text-gray-600 text-xs font-medium hover:bg-gray-50 cursor-pointer">
                Close
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 bg-gray-50">
            {MESSAGES.map((m, i) => (
              <div key={i} className={`flex ${m.from === 'ai' ? 'justify-start' : 'justify-end'}`}>
                <div
                  className={`max-w-[90%] sm:max-w-[75%] px-3 sm:px-4 py-2.5 rounded-2xl text-sm ${
                    m.from === 'ai'
          ...
          <div className="p-3 sm:p-4 border-t border-gray-100 flex gap-2">
            <input
              ...
              className="flex-1 min-w-0 px-3 sm:px-4 py-2.5 text-sm border border-gray-200 rounded-lg focus:outline-none focus:border-cyan-500"
            />
            <button type="button" className="px-3 sm:px-4 py-2.5 rounded-lg bg-cyan-500 text-white text-sm font-medium hover:bg-cyan-600 cursor-pointer shrink-0">
              Send
            </button>
          </div>
        </div>
